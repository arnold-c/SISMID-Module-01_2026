...
flu <- rio::import(
    "https://raw.githubusercontent.com/arnold-c/SISMID-Module-01_2026/main/data/flu.csv"
)
niamey <- rio::import(
  "https://raw.githubusercontent.com/arnold-c/SISMID-Module-01_2026/main/data/niamey.csv"
)
...
Then optimize $\beta$, $\gamma$, $S_0$, and $I_0$ for each site.
...
Finally, plot the fitted biweekly incidence against the observed data.
...
ggplot(niamey_predictions_g, aes(x = biweek, group = site)) +
  ...
  geom_point(aes(y = cases, color = site), size = 4, alpha = 0.8) +
  geom_line(aes(y = incidence), color = "black") +
  scale_color_manual(values = niamey_site_colors) +
  facet_wrap(
    ~site,
    ncol = 1,
    scales = "free_y",
...
First we modify the `sse_sir_g` function so that the fixed initial condition values are supplied separately from the parameters optimized by `optim()`.
Here we fix $S_0 = 5000$ and/or $I_0 = 1$, the same values used as starting values in the previous exercise.
...
Fixing either $S_0$ or $I_0$ reduces the number of free parameters, so the remaining parameters must compensate for any mismatch between the fixed value and the data.
Fixing both initial conditions is the most restrictive option: it is easier to interpret, but it usually gives a worse fit than estimating both initial conditions from the outbreak data.
...
First we modify the `sse_sir_g` function so that the fixed initial
condition values are supplied separately from the parameters optimized by
`optim()`.
Here we fix $S_0 = 5000$ and/or $I_0 = 1$, the same values used as starting
values in the previous exercise.
...
Now we can fit three versions of the model: one with $S_0$ fixed, one with
$I_0$ fixed, and one with both initial conditions fixed.
...
Fixing either $S_0$ or $I_0$ reduces the number of free parameters, so the
remaining parameters must compensate for any mismatch between the fixed value
and the data.
Fixing both initial conditions is the most restrictive option: it is easier to
interpret, but it usually gives a worse fit than estimating both initial
conditions from the outbreak data.
